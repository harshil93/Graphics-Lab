...
void setupMenus(HWND hwnd)
{
  ...
  switch (gShpData.drawMode)
  {
    case READY_MODE :
	  ...
	  EnableMenuItem(hMenu, ID_REFLECT, MF_BYCOMMAND | MF_GRAYED);
      break;
    case DRAW_MODE :
    case DRAWN_MODE :
      ...
      if (gShpData.drawMode == DRAWN_MODE)
      {
		...
		EnableMenuItem(hMenu, ID_REFLECT, MF_BYCOMMAND | MF_ENABLED);
      }
      break;
...
void processCommand(int cmd, HWND hwnd)
{
  ...
  switch(cmd)
  {
    ...
    case ID_SPHERE:
      ...
      break;
	case ID_REFLECT:
		setDrawMode(REFLECT_MODE, hwnd);
		if (DialogBox(NULL, "REFLECT", hwnd,
			(DLGPROC)DlgReflect) == TRUE)
		{
			eraseShape();
			drawShape();
			showStatusOfDrawing();
...
void showStatusOfDrawing()
{
  ...
  switch(gShpData.drawMode)
  {
    ...
    case DRAWN_MODE:
      break;
	case REFLECT_MODE:
		//sprintf_s(szText, "%s Reflection about plane specified by points P1(%lf,%lf,%lf); P2(%lf,%lf,%lf); P3(%lf,%lf,%lf)",
		//	szShapeType[gShpData.shape],
		//	gShpData.p1x, gShpData.p1y, gShpData.p1z,
		//	gShpData.p2x, gShpData.p2y, gShpData.p2z,
		//	gShpData.p3x, gShpData.p3y, gShpData.p3z);
		SetDlgItemText(gShpData.hwndStatus, ID_VARLBL0, szText);
		break;
    case TRANSLATE_MODE:
      sprintf_s(szText,"%s Translated by: X = %d, Y = %d, Z = %d",
             szShapeType[gShpData.shape],
                                ...
                                WPARAM more,LPARAM pos)
{
  ...
  {
    case WM_INITDIALOG:
        SetWindowText(hdlg, "Input: Data for Reflection");
        SetDlgItemInt(hdlg,ID_VARP1X, 0, 1);
		SetDlgItemInt(hdlg, ID_VARP1Y, 0, 1);
		SetDlgItemInt(hdlg, ID_VARP1Z, 0, 1);
		SetDlgItemInt(hdlg, ID_VARP2X, 0, 1);
		SetDlgItemInt(hdlg, ID_VARP2Y, 0, 1);
		SetDlgItemInt(hdlg, ID_VARP2Z, 0, 1);
		SetDlgItemInt(hdlg, ID_VARP3X, 0, 1);
		SetDlgItemInt(hdlg, ID_VARP3Y, 0, 1);
		SetDlgItemInt(hdlg, ID_VARP3Z, 0, 1);
      break;
    case WM_COMMAND:
      switch(more)
      {  
        case ID_DRAW:
			GetDlgItemText(hdlg, ID_VARP1X, str, 20);
          gShpData.p1x =  atof(str); /*convert the text into number*/
		  GetDlgItemText(hdlg, ID_VARP1Y, str, 20);
          gShpData.p1y =  atof(str); /*convert the text into number*/
		  GetDlgItemText(hdlg, ID_VARP1Z, str, 20);
          gShpData.p1z =  atof(str); /*convert the text into number*/

		  GetDlgItemText(hdlg, ID_VARP2X, str, 20);
		  gShpData.p2x =  atof(str); /*convert the text into number*/
		  GetDlgItemText(hdlg, ID_VARP2Y, str, 20);
		  gShpData.p2y =  atof(str); /*convert the text into number*/
		  GetDlgItemText(hdlg, ID_VARP2Z, str, 20);
		  gShpData.p2z =  atof(str); /*convert the text into number*/

		  GetDlgItemText(hdlg, ID_VARP3X, str, 20);
		  gShpData.p3x =  atof(str); /*convert the text into number*/
		  GetDlgItemText(hdlg, ID_VARP3Y, str, 20);
		  gShpData.p3y =  atof(str); /*convert the text into number*/
		  GetDlgItemText(hdlg, ID_VARP3Z, str, 20);
		  gShpData.p3z =  atof(str); /*convert the text into number*/

          EndDialog(hdlg,TRUE);
          break;
        case ID_CANCEL:
          gShpData.p1x = gShpData.p1y = gShpData.p1z = 0;
		  gShpData.p2x = gShpData.p2y = gShpData.p2z = 0;
		  gShpData.p3x = gShpData.p3y = gShpData.p3z = 0;
